...
void print_matrix(double* buffer,int m, int n)
{
    for (int i=0;i<m;i++)
    {
        for (int j=0;j<n;j++)
...
void householder(double* A, double* Z,double *d,double* e,int m, int n, int rank)
{
    ...
    for (int i=0;i<m;i++)
    {
        for (int j=0;j<n;j++)
        {
            Z[matind(i,j,n)]= (j==i+m*rank) ? (1) : (0);
        }
    }
    // printf("Rank inside householder =%d\n",rank);
    for (int k=0;k<n-2;k++)
    {
        double s=formW(W,A,k,m,n,rank);
        double c=formV(V,A,W,m,n,rank);
        formQ(Q,V,W,c,m,n,rank);
        formA(A,Q,W,m,n,rank);
        formZ(Z,W,m,n,rank);
        int cur_rank=k/m;
        if (cur_rank==rank)
        {
            d[k]=A[(matind(k-m*rank,k,n))];
            e[k]=A[(matind(k-m*rank,k+1,n))];
        }
        // break;
    ...
    if (rank==n/m-1)
    {
        d[n-2]=A[matind(n-2-m*rank,n-2,n)];
        d[n-1]=A[matind(n-1-m*rank,n-1,n)];
        e[n-2]=A[matind(n-2-m*rank,n-1,n)];
        e[n-1]=0;
    }
    MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, d,m, MPI_DOUBLE, MPI_COMM_WORLD);
    MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, e,m, MPI_DOUBLE, MPI_COMM_WORLD);
    free(W);
    free(V);
    free(Q);
}
...
{    
    ...
    {    
        ...
        for (int j=k+1;j<n;j++)
        {
            s_2+=A[matind(l_row,j,n)]*A[matind(l_row,j,n)];
        }
        double sign = (A[matind(l_row,k+1,n)] >= 0.) ? 1. : -1.;
        s= sign*sqrt(s_2);
        double R=sqrt(2*s*(A[matind(l_row,k+1,n)]+s));
        W[k]=0;
        W[k+1]=(A[matind(l_row,k+1,n)]+s)/R;
        for (int j=k+2;j<n;j++)
        {
            W[j]=A[matind(l_row,j,n)]/R;
        }
    }
    MPI_Bcast(&s,1,MPI_DOUBLE,active_rank,MPI_COMM_WORLD);
    MPI_Bcast(W,n,MPI_DOUBLE,active_rank,MPI_COMM_WORLD);
    return s;
}
...
{
    double c=0;
    int ind_shift=rank*m;
    for (int i=0;i<m;i++)
    {
        V[ind_shift+i]=0;
        for (int j=0;j<n;j++)
        {
            V[ind_shift+i]+=A[matind(i,j,n)]*W[j];
        }
        c+= V[ind_shift+i]*W[ind_shift+i];
    }
    MPI_Allgather(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, V,m, MPI_DOUBLE, MPI_COMM_WORLD);
...
void formA(double* A, double* Q, double* W, int m, int n, int rank)
{
    for (int i=0;i<m;i++)
    {
        for (int j=0;j<n;j++)
        {
            A[matind(i,j,n)]-= 2*W[i+m*rank]*Q[j] + 2*W[j]*Q[i+m*rank];
        }
    }
}

//  QL iteration functions
...
void QL_iteration(double* Z, double*d,double* e, int m, int n, double tol,int max_iter)
{
    ...
    while (block_start >= 0 )
    {
        QL_iterate(Z,block_start,block_end,d,e,m,n);
